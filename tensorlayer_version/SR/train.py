...
from SR.models import RGBD_SR, RDBs_Network
...
def train(datapath):
    ...
    for epoch in range(0, initia_epoch):
        ...
        for idx in range(0, len(train_list)-batch_size, batch_size):
            ...
            b_imgs_640_d = tl.prepro.threading_data(b_depths, fn=transform_image_depth, scale=32678.)
            ...
            if step_ % 100 == 0:
                ...
                out_valid = sess.run(net_g_valid.outputs, feed_dict={t_image_valid: valid_imgs_640_d[:1]})
...
def evaluate(datapath):
    ...
    for idx in range(0, len(test_list), batch_size):
        ...
        # output = sess.run(tf.squeeze(tf.nn.relu(net_g_test.outputs * factor)), feed_dict={t_image_valid: valid_imgs_640_d[:1]})
        # cv2.imwrite(os.path.join('C:\zdj\project\python\RGBD\SR\\result', '%s-ours_hr.png' % order), output.astype(np.uint16))
        # print('')
        ...
        # print(np.max(valid_imgs_2560_d), np.min(valid_imgs_2560_d))
