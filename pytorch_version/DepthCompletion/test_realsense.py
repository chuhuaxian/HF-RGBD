...
def main():
    ...
    DATASET_NAME = 'align_kv2'
    iteration = 4
    if not os.path.exists('C:\\Users\\39796\Desktop\\%s-i%s'% (DATASET_NAME, iteration)):
        os.makedirs('C:\\Users\\39796\Desktop\\%s-i%s'% (DATASET_NAME, iteration))
    ...
    with torch.no_grad():
        ...
        test_list = open('%s_list.txt' % DATASET_NAME).readlines()
        ...
        RES_DIR= 'C:\\Users\\39796\Desktop\\align_kv2-sparse-0'
        ...
        for _input in test_loader:
            ...
            for i in range(_input.shape[0]):
                filename = test_list[idx*batch_size+i]
                order = int(filename.strip().split(',')[0].split('-')[0].split('/')[-1])
                res_cname = '%s/%s-color.jpg' % (RES_DIR, order)
                res_rname = '%s/%s-depth.png' % (RES_DIR, order)
                res_oname = '%s/%s-ours.png' % (RES_DIR, order)

                color = np.transpose(image[i, :3, :, :].detach().cpu().numpy(), (1, 2, 0))*255.
                color = color.astype(np.uint8)

                raw = image[i, 3, :, :].detach().cpu().numpy()*10000.
                raw = raw.astype(np.uint16)

                ours = output[i, 0, :, :].detach().cpu().numpy()*1000.

                ours = cv2.bilateralFilter(src=ours.astype(np.float32), d=0, sigmaColor=150, sigmaSpace=55)
                ours = cv2.bilateralFilter(src=ours.astype(np.float32), d=0, sigmaColor=150, sigmaSpace=55)
                ours = cv2.bilateralFilter(src=ours.astype(np.float32), d=0, sigmaColor=150, sigmaSpace=55)
                ours = cv2.bilateralFilter(src=ours.astype(np.float32), d=0, sigmaColor=150, sigmaSpace=55) * 10
                ours = ours.astype(np.uint16)

                cv2.imwrite(res_cname, color[:, :, (2, 1, 0)])
                cv2.imwrite(res_rname, raw)
                cv2.imwrite(res_oname, ours)
            ...
            #
            # plt.imsave('C:\\Users\\39796\Desktop\\%s-i%s\\%04d.png'% (DATASET_NAME, iteration, idx), out)
            idx += 1
