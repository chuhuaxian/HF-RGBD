...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        input_depth = np.asarray(depth.copy().resize((80, 60), Image.NEAREST).resize((640, 480), Image.BICUBIC))
...
def getTrainingTestingData(batch_size):
    data, nyu2_train = loadZipToMem('F:\\nyu_data.zip')

    transformed_training = depthDatasetMemory(data, nyu2_train, transform=getDefaultTrainTransform())
    transformed_testing = depthDatasetMemory(data, nyu2_train, transform=getNoTransform())

    return DataLoader(transformed_training, batch_size, shuffle=False), DataLoader(transformed_testing, batch_size, shuffle=False)
